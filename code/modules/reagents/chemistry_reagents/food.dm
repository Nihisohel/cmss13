...
/datum/reagent/nutriment/process_non_property_effects(mob/living/person, list/mods, delta_time)
	var/first_collective_volume = 0
	for(var/datum/reagent/nutriment/nutri in person.reagents.reagent_list)
	...
	if(first_collective_volume >= HIGH_REAGENTS_OVERDOSE)
		...
		var/mob/living/carbon/human/subject = person
		if(ishuman(person) && subject.lastpuke == FALSE && second_collective_volume >= HIGH_REAGENTS_OVERDOSE)
