...
/mob/living/carbon/human/verb/remove_your_tourniquets()
	set name = "Remove Your Tourniquets"
	set category = "IC"

	remove_tourniquets()

/mob/living/carbon/human/proc/remove_tourniquets(mob/living/carbon/human/user, part) // evil copy paste of doom
	var/mob/living/carbon/human/target = src
	...
	if(!user.action_busy)
		var/list/obj/limb/to_tighten = list()
		var/same_arm_side = FALSE
		if(user.get_limb(cur_hand).status & LIMB_DESTROYED)
			...
			if(do_after(user, HUMAN_STRIP_DELAY * user.get_skill_duration_multiplier(SKILL_MEDICAL), INTERRUPT_ALL, BUSY_ICON_GENERIC, target, INTERRUPT_MOVED, BUSY_ICON_GENERIC))
					...
					if(target_limb && (target_limb.status & LIMB_CONSTRICTED))
						if(user == target)
							if((bodypart in list("l_arm")) && (cur_hand == "l_hand"))
								same_arm_side = TRUE
								continue
							if((bodypart in list("r_arm")) && (cur_hand == "r_hand"))
								same_arm_side = TRUE
								continue
						to_tighten += target_limb
				if(!length(to_tighten))
					if(same_arm_side)
						to_chat(user, SPAN_WARNING("You need to use the opposite hand to remove the tourniquets on your arm!"))
					else
						to_chat(user, SPAN_WARNING("There are no tourniquets to remove."))
					return

				var/obj/item/stack/medical/tourniquet/new_tourniquet = new(user.loc)
				new_tourniquet.amount = 0
				...
				for(var/obj/limb/cur_limb in to_tighten)
					...
					if(!new_tourniquet.add(1))
						...
						new_tourniquet.amount = 0
						new_tourniquet.add_fingerprint(user)
						new_tourniquet.add(1)
				if(new_tourniquet.amount == 0)
					qdel(new_tourniquet)
				msg = "[user == target ? "their own":"\proper [target]'s"]"
